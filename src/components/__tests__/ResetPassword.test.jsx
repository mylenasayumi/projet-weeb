...
describe("ResetPassword", () => {
  ...
  it("resets password", async () => {
    ...
    fireEvent.change(
      screen.getByPlaceholderText("password.confirmPasswordPlaceholder"),
      {
        target: { value: "newpassword123" },
      }
    );
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "password.resetButton" })
    );
  ...
  it("shows an error if the passwords don't match.", async () => {
    ...
    expect(
      await screen.findByText("password.passwordsDoNotMatchMessage")
    ).toBeInTheDocument();
  });
  ...
  it("shows error when confirm reset password fails", async () => {
    ...
    expect(
      await screen.findByText("password.resetErrorMessage")
    ).toBeInTheDocument();
  });
  ...
  it("shows error when reset link is invalid", async () => {
    ...
    expect(
      await screen.findByText("password.invalidLinkMessage")
    ).toBeInTheDocument();
  });
  ...
  it("shows error when password is too short", async () => {
    ...
    expect(
      await screen.findByText("password.passwordTooShortMessage")
    ).toBeInTheDocument();
  });

  it("clears auth when user is authenticated", () => {
